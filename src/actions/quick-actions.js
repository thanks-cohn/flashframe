...
async function cropOptions(image) { const panel=document.createElement("div");panel.className="crop-panel";const stage=document.createElement("div");stage.className="crop-stage";const preview=image.cloneNode();preview.removeAttribute("style");const box=document.createElement("div");box.className="crop-box";stage.append(preview,box);panel.append(stage);let rect={x:.1,y:.1,width:.8,height:.8};const paint=()=>Object.assign(box.style,{left:`${rect.x*100}%`,top:`${rect.y*100}%`,width:`${rect.width*100}%`,height:`${rect.height*100}%`});paint();box.onpointerdown=(event)=>{event.preventDefault();const bounds=stage.getBoundingClientRect(),start={clientX:event.clientX,clientY:event.clientY,x:rect.x,y:rect.y};box.setPointerCapture(event.pointerId);box.onpointermove=(move)=>{rect.x=Math.max(0,Math.min(1-rect.width,start.x+(move.clientX-start.clientX)/bounds.width));rect.y=Math.max(0,Math.min(1-rect.height,start.y+(move.clientY-start.clientY)/bounds.height));paint();};box.onpointerup=()=>{box.onpointermove=null;};};const resize=document.createElement("label");resize.innerHTML='Crop size <input type="range" min="10" max="100" value="80">%';resize.querySelector("input").oninput=event=>{rect.width=rect.height=Number(event.target.value)/100;rect.x=Math.min(rect.x,1-rect.width);rect.y=Math.min(rect.y,1-rect.height);paint();};panel.append(resize);if(!await showDialog("Crop image — drag the crop box",panel))return null;return{x:Math.round(rect.x*image.naturalWidth),y:Math.round(rect.y*image.naturalHeight),width:Math.round(rect.width*image.naturalWidth),height:Math.round(rect.height*image.naturalHeight)}; }
...
register({id:"image.annotate",label:"Annotate",appliesTo:applies("image",{max:1}),async run({selection:[item]}){const text=prompt("Annotation text (leave blank for a rectangle)","");const annotation=text?{type:"text",text,x:.1,y:.15,color:"#ff0000",size:28}:{type:"rectangle",x:.1,y:.1,width:.5,height:.3,color:"#ff0000"};updateTransform(item,{annotations:[...(transforms.get(item)?.annotations||[]),annotation]});announce("Annotation added. It remains editable in the snapshot until Save As.");}});

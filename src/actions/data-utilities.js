...
import { ARCHIVE_LIMITS, archiveEntryKind, createArchiveBudget } from "./archive-safety.js";

const workspace = document.querySelector("#workspace");
const status = document.querySelector("#status");
const api = window.FrameChuteWorkspace;
const archiveRuntime = new WeakMap();
const MAX_ARCHIVE_BYTES = ARCHIVE_LIMITS.entryBytes;
...
function archiveMime(name) { const ext=name.split(".").pop().toLowerCase(); return ({png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",gif:"image/gif",webp:"image/webp",txt:"text/plain",md:"text/markdown",markdown:"text/markdown",csv:"text/csv",pdf:"application/pdf",docx:"application/vnd.openxmlformats-officedocument.wordprocessingml.document",mp4:"video/mp4",webm:"video/webm",mp3:"audio/mpeg",wav:"audio/wav"})[ext]||"application/octet-stream"; }
...
function renderArchive(block, bytes, selected="") {
  let entries;try{const budget=createArchiveBudget();entries=unzipSync(bytes,{filter:({originalSize})=>budget.inspect(originalSize)});}catch(error){archiveRuntime.delete(block);block.querySelector(".archive-entry-list")?.replaceChildren();setStatus(`Could not open archive safely: ${error.message}`);return;}
  ...
  if(block.dataset.blockType==="cbz"){const images=Object.keys(entries).filter(name=>archiveMime(name).startsWith("image/")).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));const viewer=block.querySelector(".cbz-viewer");if(images.length){let index=Math.max(0,images.indexOf(selected)),url=null;const release=()=>{if(url)URL.revokeObjectURL(url);url=null;};const show=()=>{const path=images[index];release();url=URL.createObjectURL(new Blob([entries[path]],{type:archiveMime(path)}));viewer.src=url;viewer.alt=path;block.dataset.cbzIndex=String(index);block.dataset.selectedEntry=path;block.querySelector(".cbz-position").textContent=`${index+1} / ${images.length}`;};block.addEventListener("framechute:release-resources",release,{once:true});block.querySelector(".cbz-prev").onclick=()=>{index=(index-1+images.length)%images.length;show();};block.querySelector(".cbz-next").onclick=()=>{index=(index+1)%images.length;show();};show();}}
}
async function openEntry(block,path){const runtime=archiveRuntime.get(block),bytes=runtime?.entries[path];if(!bytes)return;block.dataset.selectedEntry=path;const name=path.split("/").pop(),type=archiveMime(name),blob=new Blob([bytes],{type});window.dispatchEvent(new CustomEvent("framechute:add-result-object",{detail:{blob,name,kind:archiveEntryKind(name,type)}}));}
...
window.addEventListener("framechute:save-result-file",event=>{const{blob,name}=event.detail||{};if(blob instanceof Blob)void saveBlobAs({blob,filename:name||"result.bin",extension:(name?.split(".").pop()||"bin"),mimeType:blob.type||"application/octet-stream",description:"Extracted file"});});
